...
- [x] **Pós-git: failover OpenAI-only + UI do auditor (11/06/2026):**
  - Projeto no GitHub: github.com/VenturaVini/auditor_claude (main; chaves só no .env gitignored; 5+ commits).
  - **Episódio real**: usuário estourou o limite de gasto mensal da Anthropic ("specified API usage limits") → fix: quando Anthropic cai, a AUDITORIA usa segunda opinião de OUTRO modelo OpenAI (antes mostrava erro feio); rótulo colapsa p/ 2/2. App 100% funcional só com OpenAI. Usuário aumentou o limite no Console → Claude voltou (testado).
  - **UI do auditor (pedidos do usuário)**: blocos de código nas críticas/booster vêm RECOLHIDOS ("Ver código" via Markdown collapseCode) — usuário lê texto, código só se expandir; cada etapa do debate é um card MINIMIZÁVEL (details/summary com ▸/▾ + mini-badge concordou/discordou) — críticas intermediárias chegam minimizadas, veredito final aberto. Auditores instruídos: texto primeiro + 1 bloco de código no final + caminhos RELATIVOS (gpt-5.4 estava salvando em /mnt/data, que não existe aqui).
